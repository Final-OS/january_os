...
pub(super) fn run() {
    ...
    if dma_boundary.as_u64() != boundary_phys {
        return fail("swiotlb", "last full page below 4GB should use direct DMA address");
    }
    if swiotlb.used_slots() != base_used {
    ...
    if dma_zero.as_u64() != zero_size_phys {
        return fail("swiotlb", "zero-size high address mapping should fallback to direct address");
    }
    ...
    if dma_oversize.as_u64() != oversize_phys {
        return fail("swiotlb", "oversized request should fallback to original physical address");
    }
    ...
    if swiotlb.used_slots() != bogus_before {
        return fail("swiotlb", "unmap out-of-pool address should not change slot usage");
    }
