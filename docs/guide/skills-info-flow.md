...
3.2 测试功能完备性：tests/demo 场景必须覆盖主流程、关键分支、失败路径与恢复路径，并对预期结果进行明确断言。
3.3 测试日志详细性：tests/demo 日志必须记录步骤动作、输入、关键状态、预期结果与实际结果；失败时可直接定位步骤与原因。
3.4 测试覆盖全面性：tests/demo 必须包含非法输入、意外输入与边界条件测试，禁止仅保留“正常输入”路径。
4. 进行最小验证：
   - 代码相关：`make build`
