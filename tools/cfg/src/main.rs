...
impl Config {
    ...
    fn get(&self, key: &str) -> Option<String> {
        match key {
            ...
            "memory_model.numa.max_nodes" => {
                self.memory_model.numa.as_ref().map(|n| n.max_nodes.to_string())
            }
    ...
    fn generate_rust(&self) -> String {
        let numa_nodes = self.memory_model.numa.as_ref().map(|n| n.max_nodes).unwrap_or(8);
